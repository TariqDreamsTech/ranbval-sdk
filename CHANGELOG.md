...
- **Tamper detection for the installed package.** Every other control here guards a *secret*; this
  guards *the code doing the guarding*. Editing four lines of the installed SDK defeats all of it —
  measured directly in this project — and a compromised dependency or anything with write access
  to `site-packages` can do exactly that.

  The build records a SHA-256 for every shipped `.py` file. At the first decrypt, each is re-hashed
  and compared, and a mismatch names the file:

  ```
  RanbvalSecurityError: this installation has been modified —
  crypto/enforcement.py no longer matches the published build.
  ```

  Line endings are normalised so a CRLF checkout is not reported as tampering. Nothing else is:
  whitespace and comments are part of the file, and a manifest that ignored them could be
  satisfied by code that is not the code shipped.

  **It raises. There is no flag to soften it**, and no environment variable to skip it — asserted
  by a test that greps this module for `environ`. A modified installation defeats everything else
  here, so "carry on with a warning" is not a defensible default, and an off switch would be
  reachable by exactly the code the check exists to catch.

  A failure is reported to the Live Monitor before the raise. That is what makes the check worth
  more than the file it lives in: whoever edited the package still has to defeat the reporting,
  and a machine that stops sending healthy integrity is itself visible.

  *Honest limit:* someone who can edit `secret_string.py` can also edit this checker, or
  regenerate the manifest. No in-process check survives an attacker with write access to the
  process's own code. This raises cost and produces evidence; it is not a wall.

  The manifest is regenerated by the build and its freshness gated at pre-push, because a stale
  one would either fail every install or pass while describing something else.

  Two portability bugs surfaced on the Windows CI jobs while this landed, with the POSIX suite
  green throughout. Manifest keys were built with `str(Path.relative_to(...))`, which yields the
  platform separator — so a manifest generated on Linux never matched one regenerated on Windows
  and the freshness check was permanently stale there. Keys are now POSIX-style, which they have
  to be: the manifest ships inside the wheel and is verified wherever it is installed.

  And `gen_manifest.py` printed `✓`/`✗`, which a cp1252 console cannot encode — so it crashed with
  a `UnicodeEncodeError` *while reporting the stale manifest*, hiding the failure it was
  reporting. All four scripts are ASCII-only now, with a test that encodes each to cp1252.

### Removed

- **`RANBVAL_ALLOW_COMMITTABLE_SECRET`.** An environment variable that switched off the check
  standing between the root key and a public repository — the same pattern removed from
  `RANBVAL_SKIP_REPO_CHECK` and refused for `RANBVAL_HOST`. Adding the file to `.gitignore` is one
  line and is the actual fix. A test now asserts that no environment variable bypasses the guard.
